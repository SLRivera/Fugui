...
P2F main(V2P pixel)
{
    ...
    float3 local1 = lerp(Texture2.Sample(TexSampler, pixel.uv).rgb, Texture2.Sample(TexSampler, pixel.uv).rgb, Texture2.Sample(TexSampler, pixel.uv).rgb);
    float2 local2 = ParallaxOffset(local1.rg, local1.r, 0.850000f, 0.850000f, tangentToEyeVec);
    result.fragment = CombineRGBWithAlpha(Texture1.Sample(TexSampler, local2).rgb, Texture1.Sample(TexSampler, local2).a);
    // END GENERATED CODE
